...
import {
  changeColor,
  changeInputValue,
} from "../ReduxStore/themeSlice";
import { GoHome } from "react-icons/go";
import { Link } from "react-router-dom";

const NavigationBar = () => {
  const searchBarColor = useSelector((store) => store.theme.searchColor); // default color is bg-gray-950
  const searchInput = useSelector((store) => store.theme.inputValue); //|| ""; the search movie will store here initail value is ""
  ...
  return (
    <div className="">
      ...
      {/* Navigation bar with Black color background */}
      <div
        className="w-full h-[60px] bg-black flex-row items-center px-[3%] fixed z-10
      ...
      >
        ...
        <div
          className="w-[100%] h-full flex justify-between items-center
                        lg:w-full lg:h-full
        ...
        >
          ...
          <div
            className="w-full  bg-black text-white flex justify-around items-center bottom-0 right-0 fixed z-10
          ...
          >
            ...
            <Link to="/">
              <div
                className="flex flex-col items-center p-3 cursor-pointer hover:text-orange-400
            ...
            <Link to="/contact">
              ...
              >
                ...
                <h1 className="lg:text-sm xl:text-base text-xs">Contact</h1>
              </div>
            </Link>
          </div>
          {/* User name */}
          <div className="w-[10%] h-full flex items-center justify-end">
            <div className="w-[40px] h-[40px] bg-rose-800 rounded-full text-white flex items-center justify-center font-extrabold">
              A
            </div>
          </div>
        </div>
