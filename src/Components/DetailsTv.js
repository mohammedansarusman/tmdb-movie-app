...
import useTvVideo from "../CustomHooks/useTvVideo";
...
const DetailsTv = (props) => {
  ...
  return (
    <div
      className="w-screen min-h-screen py-20 bg-gradient-to-br from-slate-800 to-red-950 flex flex-col items-center gap-5
    ...
    >
      ...
      <div className="w-screen flex flex-col items-center gap-5">
        ...
        <h1 className="font-semibold text-xl underline underline-offset-2 text-white opacity-70">
          Production Companies
        </h1>
        <div className="flex flex-col items-center">
          {production_companies.map((item) => (
            <h1 className="text-sm text-white opacity-70" key={item.id}>
              {item.name}
            </h1>
          ))}
        </div>
